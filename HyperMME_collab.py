...
for run in range(10):
    ...
    plnlp_val_pos = np.load(f'{root_dir}/plnlp_val_pos_{run}.npy')
    plnlp_val_neg = np.load(f'{root_dir}/plnlp_val_neg_{run}.npy')
    plnlp_test_pos = np.load(f'{root_dir}/plnlp_test_pos_{run}.npy')
    plnlp_test_neg = np.load(f'{root_dir}/plnlp_test_neg_{run}.npy')

    val_pos = np.stack((agdn_val_pos, e2n_val_pos, plnlp_val_pos))
    val_neg = np.stack((agdn_val_neg, e2n_val_neg, plnlp_val_neg))
    test_pos = np.stack((agdn_test_pos, e2n_test_pos, plnlp_test_pos))
    test_neg = np.stack((agdn_test_neg, e2n_test_neg, plnlp_test_neg))
